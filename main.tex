...
\begin{definition}[Minimale Graphenmenge $\textrm{Min}(P_n)$]
Die Menge der minimalen Graphen der monotonen Graph-Eigenschaft
$P_n$ heißt:
$$\textrm{Min}(P_n) := \{ G\in P_n \ | \ G \text{ ist minimal } \}$$.
\end{definition}

Die Menge $\textrm{Min}(P_n)$ charakterisiert die
Eigenschaft vollständig, wie folgender Satz zeigt:

\begin{Satz}[$\textrm{Min}(P_n)$ legt $P_n$ eindeutig fest]
Seien $P_n, P'_n \subseteq \mathcal{G}_n$ zwei monotone 
Graph-Eigenschaften mit
$\textrm{Min}(P_n) = \textrm{Min}(P'_n)$, so gilt $P_n = P'_n$.
\end{Satz}
\begin{proof}
...
Eigenschaften als Mengen $P_n = P'_n$ folgt. \\
